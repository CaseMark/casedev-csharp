using System;
using System.Text.Json;
using CaseDev.Core;
using CaseDev.Exceptions;
using CaseDev.Models.Payments.V1.Parties;

namespace CaseDev.Tests.Models.Payments.V1.Parties;

public class PartyUpdateParamsTest : TestBase
{
    [Fact]
    public void FieldRoundtrip_Works()
    {
        var parameters = new PartyUpdateParams
        {
            ID = "id",
            AddressLine1 = "address_line1",
            AddressLine2 = "address_line2",
            City = "city",
            Country = "country",
            Email = "email",
            IsActive = true,
            Metadata = JsonSerializer.Deserialize<JsonElement>("{}"),
            Name = "name",
            Notes = "notes",
            Phone = "phone",
            PostalCode = "postal_code",
            Role = PartyUpdateParamsRole.Client,
            State = "state",
        };

        string expectedID = "id";
        string expectedAddressLine1 = "address_line1";
        string expectedAddressLine2 = "address_line2";
        string expectedCity = "city";
        string expectedCountry = "country";
        string expectedEmail = "email";
        bool expectedIsActive = true;
        JsonElement expectedMetadata = JsonSerializer.Deserialize<JsonElement>("{}");
        string expectedName = "name";
        string expectedNotes = "notes";
        string expectedPhone = "phone";
        string expectedPostalCode = "postal_code";
        ApiEnum<string, PartyUpdateParamsRole> expectedRole = PartyUpdateParamsRole.Client;
        string expectedState = "state";

        Assert.Equal(expectedID, parameters.ID);
        Assert.Equal(expectedAddressLine1, parameters.AddressLine1);
        Assert.Equal(expectedAddressLine2, parameters.AddressLine2);
        Assert.Equal(expectedCity, parameters.City);
        Assert.Equal(expectedCountry, parameters.Country);
        Assert.Equal(expectedEmail, parameters.Email);
        Assert.Equal(expectedIsActive, parameters.IsActive);
        Assert.NotNull(parameters.Metadata);
        Assert.True(JsonElement.DeepEquals(expectedMetadata, parameters.Metadata.Value));
        Assert.Equal(expectedName, parameters.Name);
        Assert.Equal(expectedNotes, parameters.Notes);
        Assert.Equal(expectedPhone, parameters.Phone);
        Assert.Equal(expectedPostalCode, parameters.PostalCode);
        Assert.Equal(expectedRole, parameters.Role);
        Assert.Equal(expectedState, parameters.State);
    }

    [Fact]
    public void OptionalNonNullableParamsUnsetAreNotSet_Works()
    {
        var parameters = new PartyUpdateParams { ID = "id" };

        Assert.Null(parameters.AddressLine1);
        Assert.False(parameters.RawBodyData.ContainsKey("address_line1"));
        Assert.Null(parameters.AddressLine2);
        Assert.False(parameters.RawBodyData.ContainsKey("address_line2"));
        Assert.Null(parameters.City);
        Assert.False(parameters.RawBodyData.ContainsKey("city"));
        Assert.Null(parameters.Country);
        Assert.False(parameters.RawBodyData.ContainsKey("country"));
        Assert.Null(parameters.Email);
        Assert.False(parameters.RawBodyData.ContainsKey("email"));
        Assert.Null(parameters.IsActive);
        Assert.False(parameters.RawBodyData.ContainsKey("is_active"));
        Assert.Null(parameters.Metadata);
        Assert.False(parameters.RawBodyData.ContainsKey("metadata"));
        Assert.Null(parameters.Name);
        Assert.False(parameters.RawBodyData.ContainsKey("name"));
        Assert.Null(parameters.Notes);
        Assert.False(parameters.RawBodyData.ContainsKey("notes"));
        Assert.Null(parameters.Phone);
        Assert.False(parameters.RawBodyData.ContainsKey("phone"));
        Assert.Null(parameters.PostalCode);
        Assert.False(parameters.RawBodyData.ContainsKey("postal_code"));
        Assert.Null(parameters.Role);
        Assert.False(parameters.RawBodyData.ContainsKey("role"));
        Assert.Null(parameters.State);
        Assert.False(parameters.RawBodyData.ContainsKey("state"));
    }

    [Fact]
    public void OptionalNonNullableParamsSetToNullAreNotSet_Works()
    {
        var parameters = new PartyUpdateParams
        {
            ID = "id",

            // Null should be interpreted as omitted for these properties
            AddressLine1 = null,
            AddressLine2 = null,
            City = null,
            Country = null,
            Email = null,
            IsActive = null,
            Metadata = null,
            Name = null,
            Notes = null,
            Phone = null,
            PostalCode = null,
            Role = null,
            State = null,
        };

        Assert.Null(parameters.AddressLine1);
        Assert.False(parameters.RawBodyData.ContainsKey("address_line1"));
        Assert.Null(parameters.AddressLine2);
        Assert.False(parameters.RawBodyData.ContainsKey("address_line2"));
        Assert.Null(parameters.City);
        Assert.False(parameters.RawBodyData.ContainsKey("city"));
        Assert.Null(parameters.Country);
        Assert.False(parameters.RawBodyData.ContainsKey("country"));
        Assert.Null(parameters.Email);
        Assert.False(parameters.RawBodyData.ContainsKey("email"));
        Assert.Null(parameters.IsActive);
        Assert.False(parameters.RawBodyData.ContainsKey("is_active"));
        Assert.Null(parameters.Metadata);
        Assert.False(parameters.RawBodyData.ContainsKey("metadata"));
        Assert.Null(parameters.Name);
        Assert.False(parameters.RawBodyData.ContainsKey("name"));
        Assert.Null(parameters.Notes);
        Assert.False(parameters.RawBodyData.ContainsKey("notes"));
        Assert.Null(parameters.Phone);
        Assert.False(parameters.RawBodyData.ContainsKey("phone"));
        Assert.Null(parameters.PostalCode);
        Assert.False(parameters.RawBodyData.ContainsKey("postal_code"));
        Assert.Null(parameters.Role);
        Assert.False(parameters.RawBodyData.ContainsKey("role"));
        Assert.Null(parameters.State);
        Assert.False(parameters.RawBodyData.ContainsKey("state"));
    }

    [Fact]
    public void Url_Works()
    {
        PartyUpdateParams parameters = new() { ID = "id" };

        var url = parameters.Url(new() { ApiKey = "My API Key" });

        Assert.Equal(new Uri("https://api.case.dev/payments/v1/parties/id"), url);
    }

    [Fact]
    public void CopyConstructor_Works()
    {
        var parameters = new PartyUpdateParams
        {
            ID = "id",
            AddressLine1 = "address_line1",
            AddressLine2 = "address_line2",
            City = "city",
            Country = "country",
            Email = "email",
            IsActive = true,
            Metadata = JsonSerializer.Deserialize<JsonElement>("{}"),
            Name = "name",
            Notes = "notes",
            Phone = "phone",
            PostalCode = "postal_code",
            Role = PartyUpdateParamsRole.Client,
            State = "state",
        };

        PartyUpdateParams copied = new(parameters);

        Assert.Equal(parameters, copied);
    }
}

public class PartyUpdateParamsRoleTest : TestBase
{
    [Theory]
    [InlineData(PartyUpdateParamsRole.Client)]
    [InlineData(PartyUpdateParamsRole.Vendor)]
    [InlineData(PartyUpdateParamsRole.Counsel)]
    [InlineData(PartyUpdateParamsRole.Expert)]
    [InlineData(PartyUpdateParamsRole.LienHolder)]
    [InlineData(PartyUpdateParamsRole.Funder)]
    [InlineData(PartyUpdateParamsRole.OpposingParty)]
    [InlineData(PartyUpdateParamsRole.Other)]
    public void Validation_Works(PartyUpdateParamsRole rawValue)
    {
        // force implicit conversion because Theory can't do that for us
        ApiEnum<string, PartyUpdateParamsRole> value = rawValue;
        value.Validate();
    }

    [Fact]
    public void InvalidEnumValidationThrows_Works()
    {
        var value = JsonSerializer.Deserialize<ApiEnum<string, PartyUpdateParamsRole>>(
            JsonSerializer.SerializeToElement("invalid value"),
            ModelBase.SerializerOptions
        );

        Assert.NotNull(value);
        Assert.Throws<CasedevInvalidDataException>(() => value.Validate());
    }

    [Theory]
    [InlineData(PartyUpdateParamsRole.Client)]
    [InlineData(PartyUpdateParamsRole.Vendor)]
    [InlineData(PartyUpdateParamsRole.Counsel)]
    [InlineData(PartyUpdateParamsRole.Expert)]
    [InlineData(PartyUpdateParamsRole.LienHolder)]
    [InlineData(PartyUpdateParamsRole.Funder)]
    [InlineData(PartyUpdateParamsRole.OpposingParty)]
    [InlineData(PartyUpdateParamsRole.Other)]
    public void SerializationRoundtrip_Works(PartyUpdateParamsRole rawValue)
    {
        // force implicit conversion because Theory can't do that for us
        ApiEnum<string, PartyUpdateParamsRole> value = rawValue;

        string json = JsonSerializer.Serialize(value, ModelBase.SerializerOptions);
        var deserialized = JsonSerializer.Deserialize<ApiEnum<string, PartyUpdateParamsRole>>(
            json,
            ModelBase.SerializerOptions
        );

        Assert.Equal(value, deserialized);
    }

    [Fact]
    public void InvalidEnumSerializationRoundtrip_Works()
    {
        var value = JsonSerializer.Deserialize<ApiEnum<string, PartyUpdateParamsRole>>(
            JsonSerializer.SerializeToElement("invalid value"),
            ModelBase.SerializerOptions
        );
        string json = JsonSerializer.Serialize(value, ModelBase.SerializerOptions);
        var deserialized = JsonSerializer.Deserialize<ApiEnum<string, PartyUpdateParamsRole>>(
            json,
            ModelBase.SerializerOptions
        );

        Assert.Equal(value, deserialized);
    }
}
