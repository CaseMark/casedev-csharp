using System;
using System.Collections.Generic;
using Casedev.Models.Legal.V1;

namespace Casedev.Tests.Models.Legal.V1;

public class V1ResearchParamsTest : TestBase
{
    [Fact]
    public void FieldRoundtrip_Works()
    {
        var parameters = new V1ResearchParams
        {
            Query = "xxx",
            AdditionalQueries = ["string"],
            Jurisdiction = "jurisdiction",
            NumResults = 1,
        };

        string expectedQuery = "xxx";
        List<string> expectedAdditionalQueries = ["string"];
        string expectedJurisdiction = "jurisdiction";
        long expectedNumResults = 1;

        Assert.Equal(expectedQuery, parameters.Query);
        Assert.NotNull(parameters.AdditionalQueries);
        Assert.Equal(expectedAdditionalQueries.Count, parameters.AdditionalQueries.Count);
        for (int i = 0; i < expectedAdditionalQueries.Count; i++)
        {
            Assert.Equal(expectedAdditionalQueries[i], parameters.AdditionalQueries[i]);
        }
        Assert.Equal(expectedJurisdiction, parameters.Jurisdiction);
        Assert.Equal(expectedNumResults, parameters.NumResults);
    }

    [Fact]
    public void OptionalNonNullableParamsUnsetAreNotSet_Works()
    {
        var parameters = new V1ResearchParams { Query = "xxx" };

        Assert.Null(parameters.AdditionalQueries);
        Assert.False(parameters.RawBodyData.ContainsKey("additionalQueries"));
        Assert.Null(parameters.Jurisdiction);
        Assert.False(parameters.RawBodyData.ContainsKey("jurisdiction"));
        Assert.Null(parameters.NumResults);
        Assert.False(parameters.RawBodyData.ContainsKey("numResults"));
    }

    [Fact]
    public void OptionalNonNullableParamsSetToNullAreNotSet_Works()
    {
        var parameters = new V1ResearchParams
        {
            Query = "xxx",

            // Null should be interpreted as omitted for these properties
            AdditionalQueries = null,
            Jurisdiction = null,
            NumResults = null,
        };

        Assert.Null(parameters.AdditionalQueries);
        Assert.False(parameters.RawBodyData.ContainsKey("additionalQueries"));
        Assert.Null(parameters.Jurisdiction);
        Assert.False(parameters.RawBodyData.ContainsKey("jurisdiction"));
        Assert.Null(parameters.NumResults);
        Assert.False(parameters.RawBodyData.ContainsKey("numResults"));
    }

    [Fact]
    public void Url_Works()
    {
        V1ResearchParams parameters = new() { Query = "xxx" };

        var url = parameters.Url(new() { ApiKey = "My API Key" });

        Assert.True(TestBase.UrisEqual(new Uri("https://api.case.dev/legal/v1/research"), url));
    }

    [Fact]
    public void CopyConstructor_Works()
    {
        var parameters = new V1ResearchParams
        {
            Query = "xxx",
            AdditionalQueries = ["string"],
            Jurisdiction = "jurisdiction",
            NumResults = 1,
        };

        V1ResearchParams copied = new(parameters);

        Assert.Equal(parameters, copied);
    }
}
