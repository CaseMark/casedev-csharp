...
/// <summary>
/// Sends a message to a Daytona-backed chat runtime and returns the complete response
/// as a single JSON body. Blocks until the assistant turn completes.
///
    ...
    public override System::Uri Url(ClientOptions options)
    {
        return new System::UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/')
                + string.Format("/agent/v2/chat/{0}/message", this.ID)
        )
        {
