...
public sealed class V1Service : IV1Service
{
    ...
    /// <inheritdoc/>
    public async Task<V1SecFilingResponse> SecFiling(
        V1SecFilingParams parameters,
        CancellationToken cancellationToken = default
    )
    {
        using var response = await this
            .WithRawResponse.SecFiling(parameters, cancellationToken)
            .ConfigureAwait(false);
        return await response.Deserialize(cancellationToken).ConfigureAwait(false);
    }
...
public sealed class V1ServiceWithRawResponse : IV1ServiceWithRawResponse
{
    ...
    /// <inheritdoc/>
    public async Task<HttpResponse<V1SecFilingResponse>> SecFiling(
        V1SecFilingParams parameters,
        CancellationToken cancellationToken = default
    )
    {
        HttpRequest<V1SecFilingParams> request = new()
        {
            Method = HttpMethod.Post,
            Params = parameters,
        };
        ...
        return new(
            ...
            {
                var deserializedResponse = await response
                    .Deserialize<V1SecFilingResponse>(token)
                    .ConfigureAwait(false);
                if (this._client.ResponseValidation)
                {
                    deserializedResponse.Validate();
