...
public sealed class RunService : IRunService
{
    ...
    /// <inheritdoc/>
    public async Task<RunListResponse> List(
...
public sealed class RunServiceWithRawResponse : IRunServiceWithRawResponse
{
    ...
    /// <inheritdoc/>
    public async Task<HttpResponse<RunListResponse>> List(
        RunListParams? parameters = null,
        CancellationToken cancellationToken = default
    )
    {
        ...
        return new(
            ...
            async (token) =>
            {
                var runs = await response.Deserialize<RunListResponse>(token).ConfigureAwait(false);
                if (this._client.ResponseValidation)
                {
                    runs.Validate();
                }
