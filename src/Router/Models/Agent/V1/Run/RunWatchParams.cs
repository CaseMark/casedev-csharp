...
/// </summary>
public record class RunWatchParams : ParamsBase
{
    ...
    public RunWatchParams() { }

#pragma warning disable CS8618
    [SetsRequiredMembers]
    public RunWatchParams(RunWatchParams runWatchParams)
        : base(runWatchParams)
    {
        this.ID = runWatchParams.ID;

        this._rawBodyData = new(runWatchParams._rawBodyData);
    }
...
#pragma warning restore CS8618
    ...
    public static RunWatchParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData,
        IReadOnlyDictionary<string, JsonElement> rawBodyData
    )
    ...
    public virtual bool Equals(RunWatchParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/')
                + string.Format("/agent/v1/run/{0}/watch", this.ID)
        )
        {
